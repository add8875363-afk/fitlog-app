...
> `.env.local` 被 `.gitignore` 擋住,**不會**上傳到 GitHub。所以 Vercel 那邊**必須手動再設一次**環境變數。

### 步驟 1:在 Vercel 設定環境變數

1. Vercel Dashboard → 你的專案 → **Settings** → **Environment Variables**
2. 把 `.env.local` 的 6 個 `VITE_FIREBASE_*` 全部 Add 進去
3. ⚠️ **值要貼 Firebase Console 給你的真值**,不是文件裡的中文說明字(例如「貼上你的apiKey值」、「apiKey_的值」這種都是佔位符,不是值)
4. 加完**一定要點每個變數的「眼睛」圖示或「⋯ → Edit」再檢查一次**,確保 Value 是像 `AIzaSy...` 這種真的英數字串

### 步驟 2:改完值要重新部署

1. Deployments 分頁 → 最新那筆 → **⋯** → **Redeploy**
2. **取消勾選** `Use existing Build Cache`(一定要!)

> 為什麼:Vite 的環境變數是在 build 時被編譯進去的,不是執行時讀。如果用舊的 build cache,等於還是用舊的值。

### 步驟 3:把部署網域加到 Firebase 授權網域

1. Firebase Console → Authentication → 內容區頂部的 **「設定」** 分頁 → **已授權的網域**
2. 點「新增網域」→ 輸入你的部署網域(例如 `xxx.vercel.app`,不用加 `https://`)

### Q: 部署版註冊時跳「網路連線失敗,請檢查網路」怎麼辦?

這個錯誤**多半不是**真的網路問題,照順序檢查:

1. **Vercel 環境變數的值是不是中文佔位符?**
   - 到 Vercel → Environment Variables → 點每個變數的眼睛圖示展開
   - 如果看到 `apiKey_的值`、`你的_API_KEY` 這種中文字 → 就是這個原因,改成真值
2. **改完值有 Redeploy 嗎(且取消 Build Cache)?** — Vite 的 env 是 build 時注入,沒重 build 就不會生效
3. **Firebase 授權網域有加部署網域嗎?**
4. **瀏覽器 Service Worker 快取舊 JS** — 因為這個 App 是 PWA,SW 會快取舊 bundle
   - 解法:F12 → **Application** → **Service Workers** → **Unregister**,或開**無痕視窗**測試
